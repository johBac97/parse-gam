...
STEPS = ["detect", "parse", "smooth", "moves", "game", "export", "visualize", "video"]
...
def __parse_args():
    ...
    parser.add_argument(
        "name", type=str, help="Output folder name (created in current directory)"
    )
    parser.add_argument(
        "--from-step",
        choices=STEPS,
    ...
    parser.add_argument("--batch", type=int, default=16, help="YOLO inference batch size")
    parser.add_argument(
        "--fps",
        type=int,
        default=5,
        help="Frames per second to extract from source video",
    )
    parser.add_argument(
        "--minutes",
        type=float,
        default=None,
        help="Only process the first N minutes of the video",
    )
...
def main():
    ...
    # 1. Extract raw frames from source video — saved to {name}/frames/
    #    These are clean, un-annotated frames used for die crop extraction and visualization.
    if start_idx <= STEPS.index("detect"):
        root_dir.mkdir(parents=True, exist_ok=True)
        frames_dir.mkdir(parents=True, exist_ok=True)
        ffmpeg_cmd = ["ffmpeg", "-y", "-i", str(args.source)]
        if args.minutes is not None:
            ffmpeg_cmd += ["-t", str(int(args.minutes * 60))]
        ffmpeg_cmd += [
            "-vf",
            f"fps={args.fps}",
        ...
        run(
            ffmpeg_cmd,
            f"Extracting frames at {args.fps} fps"
            + (f" (first {args.minutes} min)" if args.minutes else ""),
        )

    # 2. YOLO detection — runs on extracted frames, outputs to {name}/yolo/
    if start_idx <= STEPS.index("detect"):
        run(
            [
                "uv",
    ...
    # 3. Parse predictions to board states
    ...
    if start_idx <= STEPS.index("moves"):
        run(
            [
                ...
                str(moves_path),
                "--move-window",
                str(args.move_window),
            ],
            "Detecting moves",
        )

    # 6. Parse game structure (turns, boundaries, results)
    ...
    # 7. Export game record to text
    export_path = root_dir / "game.txt"
    if start_idx <= STEPS.index("export"):
        run(
            ["uv", "run", "export-game", str(game_path), str(export_path)],
            "Exporting game record",
        )
